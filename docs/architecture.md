...
3. **Per-tenant schedules — the one built.** `pg-boss`'s `pgboss.schedule` table keys on `(name, key)`, so a single queue can hold one schedule per tenant, each carrying its own `data: { tenantId }` and keyed by that tenant's id. `db/deploy.ts`'s `syncSessionGenerateSchedules` enumerates tenants **once, at deploy time, under the already-privileged `MIGRATION_DATABASE_URL` connection** — the same trusted zone `bootstrap-roles.ts`/`db/migrate.ts` already use — and registers or removes schedules to match. The worker never enumerates anything: `.work()` receives `job.data.tenantId` directly. This is why it won, decisively, over both options above: **there is no new exemption object.** No new marker, no new function, no new grant, no new policy — `isolation.test.ts` and `no-superuser-on-request-path.test.ts` cover it unmodified, because nothing about the RLS or privilege surface changed at all.
   ...
   **Formerly a known gap, closed by D2 (fix/demo-blockers):** a tenant created *between* deploys used to get no schedule until the next sync ran — exercisable as soon as `createTenant()` (`db/platform-tenant-create.ts`) became the operator-facing platform-UI path, ahead of self-serve signup (F-13–21) ever landing. `createTenant()` now calls `registerSessionsGenerateSchedule()` (`db/queue.ts`) immediately after its transaction commits, using the same `scheduleSessionsGenerate` helper this sync loop calls per tenant (`lib/jobs/sessions-generate-schedule.ts`) — one schedule shape, two call sites, instead of `db/deploy.ts` being the only place that ever wrote one. This bulk sync remains the reconciliation pass for a transient scheduler outage at creation time (logged and audited, non-fatal — see `createTenant`) and for tenants whose status changed since the last deploy.
...
**`getRosterAction` and `markAttendanceSessionAction` are scoped the same way**, not left open. Scoping the list while leaving direct access open would have been cosmetic — anyone who wants another coach's roster just types the session id into the URL, bypassing the list entirely. `sessionExistsInTenant` (tenant-membership only, the same insufficient check `listTodaySessions` used to have) is renamed `sessionVisibleToCaller` and carries the identical coach-scoping condition; `getRosterForSession` uses it too. Both return the "not visible" case identically to "does not exist" — a coach requesting another coach's session gets the same 404 a made-up id would produce, never a 403 that would confirm the id was real.

**A second, unrelated bug surfaced while wiring this up and made the fix silently no-op for every real user:** `requireDefaultCtx()`'s returned `Ctx.userId` was `session.user.id` — better-auth's own id — not the platform `users.id` that `batches.coach_id`/`sessions.coach_id` actually store. The comparison never matched, so every real coach would have been scoped to zero sessions, not their own. `resolveCtxFor` (the other `Ctx`-building path, used by `requireCtx`) already returned the correctly-resolved platform id via `resolveTenantAccessBySlug`'s `TenantAccess.userId` — the two paths silently disagreed on what `ctx.userId` means, and this codebase's own `createMember` (`createdBy: ctx.userId`) had already been relying on the wrong one, just never in a code path that got exercised with a real session. Fixed at the source: `resolveDefaultMembership` (`db/platform.ts`) now returns the resolved platform `userId` too, and `requireDefaultCtx` uses it — bringing both `Ctx`-building paths into agreement. Caught by actually running the E2E script against real Postgres and a real login, not by the unit tests, which had fabricated a matching id directly and so never exercised the real resolution path — a reminder that a scoping fix isn't proven by a test that never goes through the real auth flow it's meant to protect.
