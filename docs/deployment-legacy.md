# Historical deployment notes (superseded; do not operate from this file)

> Archived for the decision trail. Some instructions below use a manual
> `:latest` image, an untracked SSH `deploy.sh`, host port 3000 and a
> Dokploy per-Application layout. They are **not** the current Dev or
> Production procedure. Use [`deployment.md`](deployment.md) exclusively.
