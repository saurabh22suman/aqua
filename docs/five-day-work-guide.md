...
- [x] **2.7** Onboarding wizard, step 3 — invite the owner, assign role. `invite-owner-action.ts` + `invite-owner-form.tsx` + `db/tenant-invite.ts`. **Defect + fix recorded:** an owner who also coaches previously had no `persons`/`staff` row, because `inviteOwner` only inserted `users` + `tenant_memberships`. `inviteStaff` had already been fixed for coach/receptionist invites (the same `persons`+`staff` invariant — phase 3.6 audit-gap fix). PR C extracts the single `db/invite-helpers.ts:ensurePersonAndStaff` helper used by both paths, makes `inviteOwner` create a `persons` row always and a `staff` row when `staffType` is passed, and adds the form fields the operator needs to enter the name and pick a `staffType`. The two paths now cannot diverge again — the helper is the one place a `persons` row is created from an invite. `GREEN`
...
- [ ] **3.5** Staff directory — list, detail, create, **edit pending**. List ✓, detail ✓ (read-only), create ✓. Edit deferred: the staff detail page (`/owner/staff/[staffId]/page.tsx`) is read-only; edit UI is not built. Re-evaluating with the standing rule, the text "list, detail, create, edit" promises four behaviours and three are reachable. Un-marked Sep 2026 retroactive audit — see `docs/retroactive-checklist-audit.md`. `GREEN` (3 of 4)
...
- [x] **5.8** Permission matrix test — `tests/tier1/permission-matrix.test.ts` pins `requirePermission(ctx, "x.y")` (the post-assertXxx resolution — assertStaff / assertManagement / assertMembersWrite / assertEnquiriesAccess were deleted in role-gating sub-PR 2) and `isSessionExpiredForRole` against the six non-platform roles (owner, admin, coach, receptionist, accountant, worker, parent). The `(role × surface × feature)` matrix in `tests/tier1/role-gating-matrix.test.ts` exercises the same matrix against a real seeded DB. Mutation proof per review-checklist §6 caught a `receptionist → coach` swap in the enquiry roles constant; restored. `GREEN`
- [ ] **5.9** Documentation sync — the checklist now matches reality through Phase 4.9; architecture.md §7.5 already covers the keys / accent / terminology surfaces shipped in this batch; the `user_account` helper added under `db/` carries the import/no-restricted-paths rationale in its header. **RETROACTIVELY WRONG** on R.1-R.7 (the F3 audit found they were `[x]` when they should have been `[ ]`). Honest at the time of writing; not honest now. Un-marked Sep 2026 retroactive audit — see `docs/retroactive-checklist-audit.md`. Re-claim only after a fresh retroactive audit confirms the checklist matches reality, AND the F3 fixes (R.1 substitution UI, R.2 conflict warning on create) land and are marked.
- [ ] **5.10** Self-review — `docs/self-review.md`. **RETROACTIVELY WRONG** on R.1-R.7 for the same reason as 5.9. The "verification commands re-run clean" was honest at the time; the verdict didn't catch the gap the F3 audit later found. Un-marked Sep 2026 retroactive audit. Re-claim after a fresh self-review that explicitly walks every `[x]` against the F3 standing rule.
