...
The four fixes below were each landed as their own PR against the
`main` branch. They share a doc because they were discovered together
during a hands-on walkthrough of the owner surface and they share a
common shape: each is a small coherent unit that was fixed at the
narrowest scope the bug allowed, with a TDD test that pins the fix
and a mutation-proof that catches a future regression.
